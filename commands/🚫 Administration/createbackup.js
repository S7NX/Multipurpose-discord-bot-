...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
        ...
        if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
            return message.reply("<:no:1007436622733774911> **I am missing the ADMINISTRATOR Permission!**")
        }
        ...
        if(owner.id != cmduser.id) {
            return message.reply(`<:no:1007436622733774911> **You need to be the Owner of this Server!**`)
        }
        ...
        message.channel.send({
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({content: `<a:Loading:1007611463294275634> **Now saving the Backup!**\nThis could take up to 2 Minutes (belongs to your data amount)`}).catch(() => {})
                    // Create the backup
